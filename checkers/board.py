...
from checkers.toolbar import embed
from .constants import ROWS, RED, square_size, COLS, WHITE, tile, background
    ...
    def draw_squares(self, win):
        ...
        for row in range(ROWS):
            for col in range(row % 2, COLS, 2):
                win.blit(tile, (row * square_size, col * square_size, square_size, square_size))

    def put_text(self):
        white_score = tk.Label(embed, text='White:' + str(self.white_left) + "  ", font='Raleway')
        white_score.place(relx=0.02, rely=0.02)
        black_score = tk.Label(embed, text='Black:' + str(self.red_left) + "  ", font='Raleway')
        black_score.place(relx=0.52, rely=0.02)
